...
<?php
	...
	{
			if ($result1 = $mysqli->query("SELECT ID,Password FROM doctor WHERE Name='$_POST[pname]' ORDER BY ID"))
			{
			// display records if there are records to display
				if ($result1->num_rows > 0)
				{
					while ($row = $result1->fetch_object())
					{
						...
						{
							$_SESSION["DID"] = $row->ID;
							header("Location: http://localhost/hospital001/ListReservation.php");
						}
						else
							echo "Wrong Data!";
					}
