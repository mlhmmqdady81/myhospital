...
<form action="ListReservation_em.php" method="post">
<?php
...
$mysqli = new mysqli("localhost", "root", "", "hospital");
	...
	if ($result1 = $mysqli->query("SELECT SID,SDate,Name,Note FROM schedule s,patient p Where p.ID=s.PitientID ORDER BY SID"))
	{
	// display records if there are records to display
		if ($result1->num_rows > 0)
		{
			...
			while ($row = $result1->fetch_object())
			{
				...
				echo "<tr>";
				echo "<td><a href='ListReservation_em.php?delID=" . $row->SID . "'>delete</a></td>";
